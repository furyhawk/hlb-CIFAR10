...
hyp = {
    "opt": {
        ...
        * 1.0
        * bias_scaler
        / batchsize,  # TODO: How we're expressing this information feels somewhat clunky, is there maybe a better way to do this? :'))))
        "non_bias_lr": 1.15 * 1.35 * 1.0 / batchsize,
        "bias_decay": 0.85 * 4.8e-4 * batchsize / bias_scaler,
        "non_bias_decay": 0.85 * 4.8e-4 * batchsize,
...
class ConvGroup(nn.Module):
    ...
    def forward(self, x):
        ...
        if (
            self.short
        ):  # layer 2 doesn't necessarily need the residual, so we just return it.
            return x
        residual = x
        if self.se:
            mult = (
                torch.sigmoid(
                    self.se2(self.activ(self.se1(torch.mean(residual, dim=(2, 3)))))
                )
                .unsqueeze(-1)
                .unsqueeze(-1)
            )
...
class NetworkEMA(nn.Module):
    ...
    def update(self, current_net):
        with torch.no_grad():
            for ema_net_parameter, incoming_net_parameter in zip(
                self.net_ema.state_dict().values(), current_net.state_dict().values()
                ...
                if incoming_net_parameter.dtype in (torch.half, torch.float):
                    ema_net_parameter.mul_(self.decay).add_(
                        incoming_net_parameter.detach().mul(1.0 - self.decay)
...
def get_batches(data_dict, key, batchsize):
    ...
    for idx in range(num_epoch_examples // batchsize):
        if (
            ...
            ), data_dict[key]["targets"].index_select(
                0, shuffled[idx * batchsize : (idx + 1) * batchsize]
...
def main():
    ...
    lr_sched = torch.optim.lr_scheduler.OneCycleLR(
        ...
        total_steps=total_train_steps - num_low_lr_steps_for_ema,
        ...
        for epoch in range(hyp["misc"]["train_epochs"]):
            ...
            for epoch_step, (inputs, targets) in enumerate(
                ...
                loss = (
                    ...
                    .mul(loss_scale_scaler)
                    .sum()
                    .div(loss_scale_scaler)
                ...
                if epoch_step % 50 == 0:
                    train_acc = (
                        (outputs.detach().argmax(-1) == targets).float().mean().item()
                    )
                ...
                if (
                    current_steps < total_train_steps - num_low_lr_steps_for_ema - 1
                ...
                if (
                    ...
                    if (
                        net_ema is None or epoch_step < num_cooldown_before_freeze_steps
                    ):  # don't snapshot the network yet if so!
            ...
            print_training_details(
                ...
                is_final_entry=(epoch == hyp["misc"]["train_epochs"] - 1),
            )
...
if __name__ == "__main__":
    ...
    print(
        "Mean and variance:",
        (
            torch.mean(torch.stack(acc_list)),
